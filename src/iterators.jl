...
function Base.iterate(iter::Powerset, state::NTuple{N, Bool}) where N
    k = 1
    # Find smallest index k for which state[k] is false
    while k ≤ iter.n && state[k]; k += 1; end
    # If no such index exists, we're done
    k == iter.n+1 && return nothing
    ...
    new_S = Set(i for i ∈ 1:iter.n if new_state[i])
...
function Base.iterate(iter::Powerset)
